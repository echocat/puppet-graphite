...
hosts_as('default').each do |graphite_host|

  describe "graphite url prefix" do
    ...
    context 'Graphite 1.1 with url prefix' do
      ...
      it 'graphite should have received the data' do
        result = on(graphite_host, "sleep 10 && curl -s 'http://127.0.0.1/#{url_prefix}render?target=seriesByTag(\"arg=key\")&format=raw&from=-5min' | grep 'arg=key,.*42.0'")
        expect(result.exit_code).to eq 0
      end
    end
  end
